...
function placePiece(board: Board, shape: Shape, r: number, c: number): Board {
  const b = board.map(row => [...row]) as Board;
  for (let dr = 0; dr < shape.length; dr++)
    for (let dc = 0; dc < shape[dr].length; dc++)
      if (shape[dr][dc]) b[r + dr][c + dc] = 1;
  return b;
}
...
function findFullLines(board: Board): Pos[] {
  ...
  for (let c = 0; c < GRID; c++) {
    let full = true;
    for (let r = 0; r < GRID; r++) if (board[r][c] !== 1) { full = false; break; }
...
export function BlockBlastGame({ onBack }: { onBack: () => void }) {
  const addCoinsFromGame = useCoinStore(s => s.addCoinsFromGame);
  const serverBest = useCoinStore(s => s.gameBestScore);
  ...
  const [bestScore, setBestScore] = useState(serverBest);
  ...
  const endDrag = useCallback(() => {
    ...
    if (canDrop && ghostPos && canPlace(board, piece.shape, ghostPos.r, ghostPos.c)) {
      let newBoard = placePiece(board, piece.shape, ghostPos.r, ghostPos.c);
      haptic("medium");

      let cellCount = 0;
      for (const row of piece.shape) for (const c of row) if (c) cellCount++;
      ...
      if (lines.length > 0) {
        newCombo = combo + 1;
        const lineBonus = lineCount * 10 * Math.min(newCombo, 5);
        earnedPts += lineBonus;

        const coinsEarned = Math.floor(lineCount / 2);
        if (coinsEarned > 0) {
          addCoinsFromGame(coinsEarned);
    ...
    setGhostPos(null);
    setCanDrop(false);
  }, [dragIdx, ghostPos, canDrop, board, pieces, combo, addCoinsFromGame]);
    ...
    >
      ...
      <div
        ...
        style={{
          ...
          gap: GAP, padding: GAP, position: "relative",
        }}
      >
        {board.flatMap((row, r) =>
          row.map((cell, c) => {
            ...
            return (
              <div key={key} style={{
                ...
                ...(isClearing ? {
                  ...
                  background: "rgba(130,100,200,0.55)",
                  boxShadow: "inset 0 -2px 0 rgba(0,0,0,0.08), 0 2px 6px rgba(130,100,200,0.15)",
      ...
      <div style={{
        ...
        {pieces.map((piece, idx) => {
          if (piece.used) return <div key={piece.id} style={{ width: 72, height: 72 }} />;
            ...
            >
              <div style={{
                ...
                {piece.shape.flatMap((row, dr) =>
                  row.map((filled, dc) => (
                    <div key={`${dr}-${dc}`} style={{
                      width: miniCell, height: miniCell, borderRadius: 3,
      ...
      {dragIdx !== null && dragPos && dragPiece && (
        <div style={{
          position: "fixed", left: dragPos.x, top: dragPos.y - 80,
          ...
          <div style={{
            ...
            {dragPiece.shape.flatMap((row, dr) =>
              row.map((filled, dc) => (
                <div key={`${dr}-${dc}`} style={{
                  width: CELL - 2, height: CELL - 2, borderRadius: 7,
                  background: filled ? dragPiece.color : "transparent",
                  border: filled ? "1px solid rgba(255,255,255,0.50)" : "none",
                  boxShadow: filled ? `inset 0 -2px 0 rgba(0,0,0,0.10), 0 2px 8px ${dragPiece.color}44` : "none",
                }} />
              ))
            )}
          </div>
      ...
      {gameOver && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 18, fontWeight: 800, color: "rgba(0,0,0,0.70)" }}>Игра окончена</div>
            <div style={{ display: "flex", gap: 20 }}>
              ...
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 10, fontWeight: 600, color: "rgba(0,0,0,0.30)" }}>ЛУЧШИЙ</div>
                <div style={{ fontSize: 22, fontWeight: 900, color: "rgba(0,0,0,0.65)" }}>{bestScore}</div>
              </div>
            </div>
            <button onClick={restart} style={{
              ...
              onPointerDown={e => { (e.target as HTMLElement).style.transform = "scale(0.95)"; }}
