...
type Shape = boolean[][];
type CellState = 0 | 1; // 0=empty, 1=filled
...
/* ═══════════════════════════════════════════
   SHAPE DEFINITIONS — 18 unique shapes
   ═══════════════════════════════════════════ */
...
export function BlockBlastGame({ onBack }: { onBack: () => void }) {
  ...
  useEffect(() => {
    if (!anyPieceFits(board, pieces) && pieces.every(p => p.used || !anyPieceFits(board, [p]))) {
      // Only if all unused pieces can't fit
  ...
  const screenToGrid = useCallback((x: number, y: number, shape: Shape): Pos | null => {
    ...
    // Center the shape on the finger position
  ...
  const moveDrag = useCallback((x: number, y: number) => {
    ...
    // Offset upward so piece is visible above finger
  ...
  const endDrag = useCallback(() => {
    ...
    if (canDrop && ghostPos && canPlace(board, piece.shape, ghostPos.r, ghostPos.c)) {
      ...
      // Count cells in piece for score
      let cellCount = 0;
      ...
      if (lines.length > 0) {
        // Lines cleared
        newCombo = combo + 1;
        // Score: lines × 10 × combo multiplier + cell count
        const lineBonus = lineCount * 10 * Math.min(newCombo, 5);
        earnedPts += lineBonus;

        // Coins: 1 per 2 lines cleared in this move (rounded down)
    ...
    setGhostPos(null);
    setCanDrop(false);
  }, [dragIdx, ghostPos, canDrop, board, pieces, combo, addCoins]);

  // Pointer events on container
  const onPointerDown = useCallback((e: React.PointerEvent) => {
    // Handled by piece elements
  }, []);
  ...
  // Clearing cells set for quick lookup
  const clearingSet = useMemo(() => {
    const s = new Set<string>();
    for (const a of clearAnims)
      for (const c of a.cells) s.add(`${c.r},${c.c}`);
    return s;
  }, [clearAnims]);
                ...
                style={{
                  ...
                  ...(isClearing ? {
                    ...
                    background: canDrop
                      ...
                      : "rgba(255,80,80,0.15)",
                    border: canDrop
                      ? `1.5px dashed ${ghostColor}80`
                      : "1.5px dashed rgba(255,80,80,0.30)",
