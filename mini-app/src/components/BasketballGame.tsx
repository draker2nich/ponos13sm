...
// Свайп-баскетбол: проведи пальцем вверх чтобы бросить мяч в корзину.
// Корзина двигается, есть бонус за «nothing but net» (идеальный бросок).
...
export function BasketballGame({ onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      setHoopX(x => {
        let nx = x + hoopDir.current * (2.5 + Math.random() * 1.5);
        if (nx > W - HOOP_W / 2 - 10) { hoopDir.current = -1; nx = W - HOOP_W / 2 - 10; }
        if (nx < HOOP_W / 2 + 10) { hoopDir.current = 1; nx = HOOP_W / 2 + 10; }
        return nx;
      });
    }, 30);
    return () => clearInterval(t);
      ...
      setBalls(prev => {
        ...
        for (const b of prev) {
          ...
          if (ny <= HOOP_Y + 12 && ny + nvy >= HOOP_Y - 4) {
            ...
            if (dist < HOOP_W / 2 - 4) {
              // Score!
              const perfect = dist < 8;
              const pts = perfect ? 3 : 2;
              ...
              setFxs(f => [...f.slice(-4), {
                id: fid, x: nx, y: HOOP_Y,
                text: perfect ? `+${total} 🔥` : `+${total}`,
                color: perfect ? "#ffd700" : "#fff",
              }]);
  ...
  const handleStart = useCallback((cx: number, cy: number) => {
    ...
    if (ry < H * 0.5) return; // swipe must start from bottom half
  ...
  const handleEnd = useCallback((cx: number, cy: number) => {
    ...
    if (dy > -30 || dt > 800) return; // need upward swipe
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        <span>🏀 {score}</span>
        {phase === "playing" && <span style={{ color: timer <= 5 ? "#ff6b6b" : undefined }}>⏱ {timer}с</span>}
      ...
      >
        ...
        {/* Net lines */}
        <div style={{
          position: "absolute", top: HOOP_Y + 4, left: hoopX - HOOP_W / 2 + 8,
          width: HOOP_W - 16, height: 28,
