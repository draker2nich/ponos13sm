...
function haptic(type: "light" | "medium" | "error") {
  try {
    ...
    <>
      ...
      {sleeping && (
        <div style={{ position: "absolute", inset: 0, zIndex: 16, pointerEvents: "none", overflow: "hidden" }}>
          ...
          <div style={{
            position: "absolute", top: "8%", right: "12%", fontSize: 32,
...
function SleepZzz() {
  ...
  return (
    <div style={{ position: "absolute", top: "-10%", right: "-20%", zIndex: 18, pointerEvents: "none", width: 80, height: 120 }}>
      {[0, 1, 2].map(i => (
        <span key={i} style={{
...
function DraggablePet({ children, constraintsRef, isStroking, onHeartAt, petDomRef, anchored, headerRef, navRowRef, containerRef }: {
  ...
  const calcAnchorOffset = useCallback(() => {
    const el = petDomRef.current; const header = headerRef.current;
    const nav = navRowRef.current; const cont = containerRef.current;
    if (!el || !header || !nav || !cont) return null;
    const contR = cont.getBoundingClientRect(); const headerR = header.getBoundingClientRect();
    const navR = nav.getBoundingClientRect(); const elR = el.getBoundingClientRect();
  ...
  useEffect(() => {
    if (anchored) {
      const timer = setTimeout(() => { const off = calcAnchorOffset(); if (off) { x.set(off.x); y.set(off.y); } }, 40);
  ...
  useEffect(() => {
    if (isStroking && !anchored) {
      ...
      const tick = (now: number) => {
        if (now - last > 180) { last = now; const c = getPetCenter(); const a = Math.random() * Math.PI * 2; onHeartAt(c.x + Math.cos(a) * 50, c.y + Math.sin(a) * 50); }
  ...
  return (
    <motion.div drag={!anchored && !sleeping} dragConstraints={constraintsRef} dragElastic={0.08} dragMomentum={false}
      style={{ cursor: anchored || sleeping ? "default" : "grab", touchAction: "none", display: "inline-block", x: springX, y: springY, willChange: "transform", ...NOTAP }}>
      {children}
    </motion.div>
  );
}

interface Props { petId: number; onPetDeleted?: () => void }

export function HomePage({ petId, onPetDeleted }: Props) {
  ...
  const handleStroking = useCallback((v: boolean) => {
    setIsStroking(v);
    if (v && !strokeStart.current) { strokeStart.current = Date.now(); petTriggered.current = false; }
    if (!v) { strokeStart.current = null; petTriggered.current = false; }
  }, []);
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      if (strokeStart.current && !petTriggered.current) {
        if (Date.now() - strokeStart.current >= 2000) {
  ...
  const showFloat = (t: string) => { setFloatText(t); setFloatShow(true); setTimeout(() => setFloatShow(false), 1000); };
  const doAction = async (action: ActionType, msg: string) => {
    if (isCd(action)) { toast("Действие на кулдауне ⏳", "error"); haptic("error"); return; }
    ...
    <div ref={containerRef} style={{
      ...
      <div ref={petZoneRef} style={{ flex: 1, display: "flex", flexDirection: "column", position: "relative", zIndex: 5, overflow: "hidden", minHeight: 0 }}>
        ...
        <header ref={headerRef} style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, ...G.heavy, borderRadius: 999, height: PILL_H, padding: "0 12px 0 7px", flexShrink: 0, ...NOTAP }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden" }}>
          <div style={{ position: "relative" }}>
            ...
            <SleepZzz />
            <DraggablePet constraintsRef={petZoneRef} isStroking={isStroking} onHeartAt={spawnHeart} petDomRef={petDomRef} anchored={menuIsOpen} headerRef={headerRef} navRowRef={navRowRef} containerRef={containerRef}>
              <div ref={petDomRef} style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                <PetSVG mood={effectiveMood} petType={pet.pet_type} evolution={evo} isReacting={floatShow || isStroking} size="clamp(120px,32vw,180px)" />
                <div style={{ width: "clamp(44px,12vw,68px)", height: 6, background: sleeping ? "rgba(0,0,0,0.15)" : "rgba(0,0,0,0.07)", filter: "blur(5px)", borderRadius: "50%", marginTop: -2, pointerEvents: "none", transition: "background 0.6s ease" }} />
              </div>
            </DraggablePet>
          </div>

          <div style={{ position: "absolute", bottom: 10, left: 0, right: 0, display: "flex", justifyContent: "center", zIndex: 20, pointerEvents: "none", opacity: menuIsOpen || sleeping ? 0 : 1, transition: "opacity 0.15s ease" }}>
            <div style={{ pointerEvents: menuIsOpen || sleeping ? "none" : "auto" }}>
              {partner ? (
                <div style={{ display: "inline-flex", alignItems: "center", gap: 6, ...G.pill, borderRadius: 999, padding: "5px 14px", ...NOTAP }}>
                ...
                <button onClick={() => handleTab("partner")} style={{ display: "inline-flex", alignItems: "center", gap: 6, ...G.pill, border: "1px dashed rgba(0,0,0,0.13)", borderRadius: 999, padding: "6px 16px", cursor: "pointer", fontFamily: "inherit", fontSize: "clamp(11px,3vw,12px)", color: "rgba(0,0,0,0.42)", fontWeight: 600, outline: "none", ...NOTAP }}>
      ...
      <div style={{ position: "absolute", bottom: "clamp(5px,1.5vw,8px)", left: "50%", transform: "translateX(-50%)", width: 100, height: 4, background: "rgba(0,0,0,0.10)", borderRadius: 4, zIndex: 20, pointerEvents: "none" }} />

      {hearts.map(h => (
        <div key={h.id} style={{
          position: "fixed", top: h.y - 9, left: h.x - 9,
          fontSize: 18, pointerEvents: "none", zIndex: 1000, color: "#f9a8d4",
          filter: "drop-shadow(0 1px 3px rgba(249,168,212,0.4))",
          animation: "heart-float 0.8s ease-out forwards",
          "--hx": `${Math.cos(h.angle) * h.dist}px`, "--hy": `${Math.sin(h.angle) * h.dist}px`,
